...
初始思路是基于关键词词典、故障短语和部门映射规则，对报修文本进行故障分类与派单建议。例如，当文本中出现“轴承”“振动”“温升”等词汇时，将其归入机械故障；当文本出现“联锁”“DCS”“阀位反馈”等词汇时，将其归入控制或仪表相关类别。

规则方案的问题在于：

1. 化工现场文本存在大量缩写、口语化描述和同义表达，规则覆盖成本持续上升。
2. P0/P1 高风险判断往往依赖上下文组合，而不是单个关键词。
3. 处理部门与故障大类并非一一对应，同一故障现象可能因设备对象、影响范围和风险等级不同而分派到不同部门。
4. 规则系统可解释性强，但泛化能力弱，难以支撑长期迭代。

结论：规则更适合作为数据审计、泄漏短语检查和人工复核辅助，不适合作为本项目的主模型路线。

### 3.2 第二阶段：模型路线对比

在确定最终结构前，项目对四类技术路线进行了对比：

| 路线 | 代表方法 | 优势 | 局限 | 本项目定位 |
|------|------|------|------|------|
| 关键词规则 | 词典匹配、正则规则、人工映射表 | 可解释、上线成本低、便于审计 | 维护成本高，难以覆盖同义表达和隐含风险 | 用于数据审计与泄漏检查，不作为主模型 |
| 传统机器学习 | TF-IDF / 字符 n-gram + Naive Bayes / Linear SVM | 训练快、依赖少、适合快速验证工程闭环 | 对长距离语义和上下文组合建模能力有限 | 作为公开仓库默认 baseline |
| 预训练语言模型 | Chinese BERT / MacBERT / RoBERTa-wwm-ext | 能建模上下文语义，适合复杂报修表达 | 训练成本更高，需要 GPU 与模型缓存 | 作为正式语义建模路线 |
| 大语言模型分类 | Prompt 分类、少样本分类、外部推理接口 | 冷启动快，可解释输出丰富 | 成本、稳定性、数据安全和批量推理可控性需额外评估 | 可作为后续质检或人工复核辅助 |

最终决策：采用“轻量 baseline + BERT 多任务模型入口”的双路线设计。baseline 用于快速验证数据、脚本和评估闭环；BERT 路线用于后续在授权企业数据上进行正式语义建模。

### 3.3 第三阶段：从单任务分类到多任务联合建模

初始可行方案是分别训练三套分类器：一套预测故障大类，一套预测停机风险等级，一套预测处理部门。该方案实现简单，但会忽略三个任务之间的业务关联。

本项目的关键调整是将单条报修文本同时映射为三个结构化标签：
...
1. 故障大类与处理部门存在强关联，例如机械故障更可能进入机械维修队列，控制系统故障更可能进入自动化工程师队列。
2. 停机风险等级会影响派单优先级，也会影响后续人工复核策略。
3. 三任务联合输出更接近真实派单场景，比单独预测某一个标签更有业务意义。
4. BERT 方案可以通过共享编码器复用文本语义，再由三个分类头分别学习任务边界。

当前公开 baseline 在实现上采用同一字符 n-gram 特征逻辑，并为三个任务分别训练轻量分类器；BERT 入口采用共享中文 BERT 编码器和三个分类头，执行 encoder 与分类头的全量微调。

### 3.4 第四阶段：数据切分策略调整

如果仅按照单一标签切分数据，例如只按 `fault_category` 分层，可能造成 `risk_level` 和 `department` 在训练集、验证集、测试集中的联合分布发生偏移。对于多任务派单系统，这会直接影响验证指标的可信度。

因此项目在 Step 3 中使用三任务组合标签进行分层切分：
...
第一次设计评估指标时，容易只关注整体 accuracy。但在化工报修场景中，不同错误的业务代价并不相同：P0/P1 高风险工单被漏判，比普通 P2/P3 工单误分更值得关注；处理部门误分会带来转派和等待成本；三任务中任一任务出错，都可能影响派单决策。
...
结论：本项目不是单纯追求一个最高准确率，而是围绕派单风险、复核成本和高风险工单召回建立评估体系。

### 3.6 第六阶段：从离线分类到人机协同闭环

离线模型只能完成“文本到标签”的第一步。面向企业落地时，更合理的方式是将模型输出作为派单建议，而不是直接替代人工判断。
...
该设计保留了人工审核在安全与生产场景中的最终控制权，同时让模型承担高频、重复、标准化的初筛工作。后续如果接入真实企业工单，应优先完善低置信度样本回流、P0/P1 误分审计和部门误派分析。
