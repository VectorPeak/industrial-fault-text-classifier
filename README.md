...
工单文本本身很难直接用于统计和派单。只有先把它拆成故障大类、停机风险等级和处理部门这些稳定字段，后续的风险排序、维修统计、设备画像才有可靠的数据入口。

**2. 多任务建模更接近真实派单过程**

单独预测故障类型只能回答“发生了什么”。实际派单还需要判断“有多紧急”和“应该由谁处理”，所以故障大类、风险等级、处理部门放在同一个建模流程里更自然。

**3. 清洗和切分比想象中更关键**

空字段、完全重复、同一文本对应多组标签、疑似标签泄漏，这些问题都会直接污染监督信号。对工单文本来说，数据审计、冲突剔除和组合标签分层切分，和模型选型一样重要。

**4. 高风险召回要放在整体准确率前面**

化工场景里，P0/P1 工单漏判的代价通常高于普通类别误分。因此评估时不能只看 accuracy，还要单独看 P0/P1 recall、macro-F1、三任务全对率和典型混淆组合。

**5. baseline 的意义是校验工程闭环**

字符 n-gram Naive Bayes 不是最终模型上限，但它训练快、依赖少、容易复现。用它先跑通数据读取、标签映射、切分、训练、评估和推理，比一开始就堆复杂模型更稳。

**6. BERT 更适合作为正式语义建模路线**

当数据授权、脱敏、标注规范和评估口径都稳定后，再比较 Chinese BERT、MacBERT、RoBERTa-wwm-ext 等预训练模型更有意义。否则模型变复杂，只会把数据问题藏得更深。

**7. 持续反馈比一次训练更重要**

低置信度样本、高风险误分样本、部门转派样本都应该进入人工复核。把修正结果回流训练集，逐步稳定标注规范和反馈机制，才是这类系统真正能长期落地的关键。
