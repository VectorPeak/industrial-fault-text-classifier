...
文本结构化先于模型训练：报修文本只有先沉淀为故障大类、停机风险等级和处理部门，后续派单、统计和设备画像才有稳定输入。

多任务建模更贴近真实派单：故障类型回答“发生了什么”，风险等级回答“有多紧急”，处理部门回答“谁来处理”，三者应放在同一条业务判断链路中。

数据质量决定监督信号上限：空字段、重复样本、同文本多标签冲突和标签泄漏会直接污染训练目标，清洗、审计和组合标签分层切分必须前置。

高风险召回和反馈闭环必须保留：P0/P1 漏判比普通误分代价更高，评估时要单独看高风险召回；低置信度、误分和转派样本也要进入复核并回流训练集。
